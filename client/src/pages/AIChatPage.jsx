...
export function AIChatPage({ setActiveTab }) {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-slate-50/50 dark:bg-slate-950/40 relative select-text">
      ...
      <div className="h-14 border-b border-slate-200 dark:border-slate-800/80 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md px-4 sm:px-6 flex items-center justify-between gap-3 flex-shrink-0 z-10 select-none">
        ...
        <div className="flex items-center gap-2 overflow-x-auto">
          ...
          >
            ...
            <span>₱{Number(metrics.remaining_today || 0).toLocaleString()} {isTL ? 'Today' : 'Today'}</span>
          </button>
          ...
          >
            ...
            <span>{metrics.days_until_payday || 0}d {isTL ? 'Sahod' : 'Payday'}</span>
          </button>
          ...
          >
            ...
            <span>{obligations.filter(o => !o.is_paid).length} {isTL ? 'Bills' : 'Bills'}</span>
          </button>
      ...
      {/* 2. Topic Channels Bar (Wants, Bills, Payday, Allowances, Savings, General) */}
      <div className="bg-white/50 dark:bg-slate-900/50 border-b border-slate-200/80 dark:border-slate-800/60 px-4 py-2 flex items-center gap-1.5 overflow-x-auto select-none flex-shrink-0">
        <span className="text-[10px] uppercase font-black text-slate-400 dark:text-slate-500 mr-1 hidden sm:inline">
          {isTL ? 'Topic:' : 'Topic:'}
        </span>
        {CHANNELS.map(ch => {
          ...
          return (
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-extrabold transition flex items-center gap-1.5 whitespace-nowrap cursor-pointer ${
                isActive
                  ? 'bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 shadow-sm ring-1 ring-slate-800 dark:ring-slate-200'
                  : 'bg-white dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 border border-slate-200/80 dark:border-slate-700/60 hover:bg-slate-100 dark:hover:bg-slate-700'
              }`}
            >
              <span>{ch.icon}</span>
              <span>{isTL ? ch.labelTl : ch.labelEn}</span>
            </button>
          );
        })}
      </div>

      {/* 3. Full-Height Message Stream */}
